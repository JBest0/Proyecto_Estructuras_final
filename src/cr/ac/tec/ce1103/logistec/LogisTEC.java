...
public class LogisTEC {
    ...
    /** Rutas calculadas por camión (para la UI). */
    private ArrayList<ArrayList<Integer>> rutasCamiones = new ArrayList<>();

    /** Nombres de los camiones en el mismo orden que rutasCamiones. */
    private ArrayList<String> nombresCamiones = new ArrayList<>();
    ...
    /**
     * Verifica la alcanzabilidad de los paquetes desde el depósito con Warshall.
     * Marca como rechazados los paquetes con destino inalcanzable.
     */
        ...
        try {
            ...
            for (int i = 0; i < data.paquetes.size(); i++) {
                ...
                if (idx != null && !reachable[data.depotIndex][idx]) {
                    p.marcarRechazado();
                    rechazados++;
                    System.out.println("  Inalcanzable: " + p.getId()
                        + " → " + p.getDestino());
                }
            }
            System.out.println("Paquetes inalcanzables marcados: " + rechazados);
        } catch (UnsupportedOperationException e) {
            System.out.println("[Warshall] Pendiente de implementación.");
        }
        System.out.println();
    }
    ...
    private void calcularMatrizDistancias() {
        ...
        try {
            ...
            System.out.println("[FloydWarshall] Pendiente de implementación.");
    ...
    private void generarMSTPrim() {
        ...
        try {
            ...
            Prim.printMST(mst);
            System.out.printf("Tiempo Prim: %.3f ms%n", tiempo / 1_000_000.0);
        } catch (Exception e) {
    ...
    /**
     * Construye el MST usando Kruskal y compara el costo con Prim.
     */
    private void generarMSTKruskal() {
        ...
        try {
            ...
            long tiempo = System.nanoTime() - inicio;
            Prim.printMST(mst);
            System.out.printf("Tiempo Kruskal: %.3f ms%n", tiempo / 1_000_000.0);
        } catch (UnsupportedOperationException e) {
            System.out.println("[Kruskal] Pendiente de implementación.");
        }
        System.out.println();
    }

    /**
     * Retorna lista de aristas únicas del grafo (sin duplicados).
     */
        ...
        for (int i = 0; i < data.paquetes.size(); i++) {
            ...
            if (p.isRechazado()) {
                ...
                System.out.println("  " + p.getId()
                    + " → camión " + paqueteACamion.get(p.getId()));
            }
        }
    ...
    /**
     * Planifica la ruta de cada camión con NN y MST-based.
     * Guarda la mejor ruta de cada camión en rutasCamiones para la UI.
     */
    private void planificarRutas() {
        ...
        for (int i = 0; i < data.camiones.size(); i++) {
            ...
            // Guardar para la UI
            rutasCamiones.add(mejorRuta);
            nombresCamiones.add(t.getId());
        }
    }

    /**
     * Convierte una ruta de índices a nombres de vértice legibles.
     */
    ...
    private void generarReporte() {
        ...
        for (int i = 0; i < data.paquetes.size(); i++) {
            ...
            if (p.isRechazado()) {
                rechazados++;
                System.out.println("  Rechazado: " + p.getId()
                    + " → " + p.getDestino()
    ...
    /**
     * Muestra la interfaz gráfica Swing con el grafo y las rutas de cada camión.
     */
    private void mostrarUI() {
        try {
            cr.ac.tec.ce1103.logistec.ui.LogisTECFrame frame =
                new cr.ac.tec.ce1103.logistec.ui.LogisTECFrame(
                    data, rutasCamiones, nombresCamiones);
            frame.setVisible(true);
        } catch (Exception e) {
            System.out.println("[UI] No se pudo iniciar: " + e.getMessage());
        }
    }
     ...
     *
     * @param args args[0] debe ser la ruta al archivo JSON de configuración
     */
    public static void main(String[] args) {
        if (args.length < 1) {
            System.out.println(
                "Uso: java cr.ac.tec.ce1103.logistec.LogisTEC <archivo.json>");
            return;
        }
